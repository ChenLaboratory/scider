...
We can then use the `posSelRegion` to save the ROI in the metadata of the SpatialExperiment object.
...
Or we can subset on specific density level with parameter `sub_level`.
...
We can then annotate cells by their locations within each contour using function `contourAssign`.


```{r}
spe <- contourAssign(spe, coi = "breast cancer")
```


```{r}
plotSpatial(spe, color = at_level_breast_cancer, alpha = 0.5)
```

We can visualise cell type composition per level.

```{r}
dat <- as.data.frame(colData(spe))[, c("cell_type", "at_level_breast_cancer")]

dat <- dat[dat$cell_type != "Breast cancer",]

cell_count <- table(dat$at_level_breast_cancer, dat$cell_type)

cell_count <- cell_count[rowSums(cell_count) != 0,]

cell_prop <- as.data.frame(prop.table(cell_count, margin = 1))

colnames(cell_prop) <- c("at_level_breast_cancer", 
...
ggplot(cell_prop, aes(x = at_level_breast_cancer, 
                       y = Proportion, fill = cell_type)) +
  geom_bar(stat = "identity") + 
  scale_fill_manual("Cell type", values = col.p) + 
  ggtitle("Cell type composition at each breast cancer density level")
```
...
```{r}
pdat <- cbind(as.data.frame(colData(spe)), as.data.frame(spatialCoords(spe)))


plotSpatial(spe) +
  geom_path(data = spe@metadata$breast_cancer_contour, 
            aes(x = x, y = y, group = group, color = level_factor))
```
