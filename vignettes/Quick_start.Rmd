...
## Comparison between T cells from low bc level and T cells from high bc level

Excluding level 0.

```{r}

spe2 <- spe[, colData(spe)$at_level_breast_cancer != "0"]

colData(spe2)$Group <- ifelse(colData(spe2)$at_level_breast_cancer %in% as.character(seq(4)),
                              "low", "high")


cdat <- as.data.frame(colData(spe2))[,c("cell_type","at_rois","Group")] 

cdat <- as.data.frame(apply(cdat, 2, function(x) gsub(" ","_",x)))

```


```{r}
factors <- c("cell_type","at_rois","Group")

f <- as.formula(paste("~ 0 +", paste(factors, collapse = ":")))

mm <- model.matrix(f, data = cdat)
...
```{r}
data.frame("sample_id" = paste0("Ps", seq(dim(psm)[2])),
           "id" = colnames(psm))
```
