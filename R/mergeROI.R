...
#'
mergeROI <- function(spe, merge.list,
                     id = "component", rename = FALSE) {
    # check ROI exists
    ...
    if (any(list_sizes < 2L)) {
        stop("Each vector in the list must have
         at least 2 unique ROI IDs to merge!")
    }
