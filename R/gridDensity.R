...
gridDensity <- function(spe,
  ...
  for (ii in seq_len(length(coi))) {
    ...
    if (is.null(spe@metadata$grid_density)) {
      ...
      spe@metadata$grid_density$node_x <- rep(seq_len(ngrid.x), each = ngrid.y) # horizontal ind
      spe@metadata$grid_density$node_y <- rep(seq_len(ngrid.y), ngrid.x) # vertical ind
