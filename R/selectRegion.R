  ...
  server <- function(input, output) {
    ...
    output$scatterplot <- renderPlotly({
      ...
      if (!is.null(color_var)) {
        if (is.numeric(data[[color_var]])) {
          ...
          color_palette <- grDevices::colorRampPalette(col.spec)(nlevels(factor(data[[color_var]])))
          p <- add_markers(p,
    ...
    observeEvent(input$export_region, {
      ...
      if (!is.null(sel_points)) {
        ...
        message("Selected region exported as 'sel_region' in the global environment.\n")
      }
    })
  }
