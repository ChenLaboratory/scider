#' Given a 'SpatialExperiment' data object, create pseudo-bulk samples using
#' the colData information and return a DGEList object
#' 
#' @param spe A SpatialExperiment object.
#' @param by.group Logical. Whether to perform pseudo-bulking by group. TRUE by default.
#' @param group.id Character. The column name of the colData(spe) that contains the group information. Default to 'cell_type'.
#' @param by.roi Logical. Whether to perform pseudo-bulking by ROI. TRUE by default.
#' @param roi.only Logical. Whether to filter out pseudo-bulk samples formed by cells not in any ROIs. TRUE by default.
#' @param contour Character. The name of the group or cell type on which the contour level is computed. 
#' If NULL, then no pseudo-bulking will be performed based on contour level. Default to NULL.
#'  
#' @return An edgeR::DGEList object where each library (column) is a pseudo-bulk sample.
#' @export
#'
#' @examples
#'
...
spe2PB <- function(spe,
	...
	if(!requireNamespace("SpatialExperiment", quietly=TRUE))
		stop("SpatialExperiment is required but is not installed (or can't be loaded)")
...
#   Check 'colData'
    ...
    if(!is.null(contour)){
        ...
        if(! cont %in% names(cData)){
            stop(paste(contour, " contour level is not found in colData of spe."))
        }
        else{
            clvl <- paste0("Lv", cData[, cont])
            if(length(table(clvl)) == 1) 
                warning("Only 1 contour level found. Please check whether the contour information is specified correctly.")
        }
    }

    if(is.null(grp) & is.null(rois) & is.null(clvl))
        stop("At least one of the following is required: group, ROI, or contour information.")

#	Check gene information
    if(ncol(SummarizedExperiment::rowData(spe))==0){
        genes <- data.frame(genes=rownames(spe))
    } else {
        genes <- as.data.frame(spe@rowData)
    }
...
#   Pseudo-bulk sample information
    sample.pb <- data.frame()[1:ncol(counts.pb), ]
...
#   DGEList
	dge <- edgeR::DGEList(counts=as.matrix(counts.pb), samples=sample.pb, genes=genes)
