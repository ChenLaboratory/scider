...
contour2sf <- function(spe, contour, coi, cutoff) {
  ...
  clines_lev_sf <- lapply(names(all_clines_sf), function(pp) {
    ...
    if (any(bbox == lims)) {
      ...
      if (any(sapply(inds, length) == 0L)) {
        ...
        regions_tmp <- regions
        regions_tmp[ind_buffer, ] <- sf::st_buffer(regions_tmp[ind_buffer, ],
          dist = mean(
  ...
  if (!is.null(areas_down)) {
    ...
    if (nrow(areas_down_out) > 0L) {
      ...
      areas_down_out <- areas_down_out[sf::st_geometry_type(areas_down_out) %in% c(
        "POLYGON",
        "MULTIPOLYGON"
      ), ]
      # check if downs are really down
      areas_down_out_code <- sapply(seq_len(nrow(areas_down_out)), function(xx) {
