...
#' @param sub.level Numeric vector of length 1 or 2, identifies which
...
plotContourRegion <- function(spe,
    ...
    if (any(!(sub.level %in% levs))) {
        stop("Non-existing level(s) in sub.level! Check
         unique(spe@metadata$`contour_data`$level).")
    }
    ...
    if (length(sub.level) == 1L) {
        density_at_level <- unique(
            contour_data$cutoff[contour_data$level == sub.level]
        )
    }
    if (length(sub.level) == 2L) {
        density_at_level <- unique(
            contour_data$cutoff[contour_data$level %in% sub.level]
        )
    ...
    if (overlay == "cell") {
        ...
        colnames(dens_df)[
            which(colnames(dens_df) == coi_clean_density)
        ] <- "density"
        if (length(sub.level) == 1L) {
        ...
        if (length(sub.level) == 2L) {
            kp_grids <- dens_df$density >= density_at_level[1] &
                dens_df$density < density_at_level[2]
