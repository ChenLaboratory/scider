...
adjacent_grids_with_corner <- function (node_xx, node_yy) {
  ...
  all_yy <- c(pmax(node_yy - 1, 1), node_yy, pmin(node_yy + 1, spe@metadata$grid_info$dims[2]))
  all_yy <- all_yy[!duplicated(all_yy)]
  all_cells <- paste(rep(all_xx, each = length(all_yy)), rep(all_yy, length(all_xx)), sep = "-")
  all_cells <- all_cells[all_cells != center_cell]
  immediate_cells <- paste(c(node_xx, node_xx, pmax(node_xx - 1, 1), 
                             pmin(node_xx + 1, spe@metadata$grid_info$dims[1])), 
                           c(pmax(node_yy - 1, 1), 
  ...
  data.frame(node1 = rep(center_cell, length(all_cells) + 1), 
             node2 = c(center_cell, immediate_cells, corner_cells), 
             class = c("center", rep("immediate", length(immediate_cells)), rep("corner", length(corner_cells))))
}
